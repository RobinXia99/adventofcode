const puzzleInput = `shzsFcPssFhjFssBzdpRcNHNZrpdJdJVJZ
...
const priorityList = [];
for (let i = 0; i < 52; i++) {
  priorityList.push(`${characters[i]}${i + 1}`);
}
...
const solve = (data) => {
  ...
  data.forEach((bag) => {
    ...
    const partTwo = bag.slice(bag.length / 2, bag.length);

    priorityList.forEach((priority) => {
      if (partOne.includes(priority[0]) && partTwo.includes(priority[0])) {
        sum += Number(priority.slice(1, priority.length));
      }
    });
  });
